package exercise.ch1.topic3;

/**
 * Ring buffer. A ring buffer, or circular queue, is homework.a FIFO data structure of homework.a fixed
 * size N. It is useful for transferring data between asynchronous processes or for storing
 * log files. When the buffer is empty, the consumer waits until data is deposited; when the
 * buffer is full, the producer waits to deposit data. Develop an API for homework.a RingBuffer and
 * an implementation that uses an array representation (with circular wrap-around).
 */

public class E10339RingBufferFalse {

}
