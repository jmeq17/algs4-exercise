...
public class Percolation {
    ...
    private void full(int row1, int col1, int row2, int col2) {
        char t = isOpen[id.find(toIndex(row2, col2))];
        char r = isOpen[id.find(toIndex(row1, col1))];

        union(row1, col1, row2, col2);

        if (t != 'f' && r != 'f') {
            if (t == 'n' || r == 'n') isOpen[id.find(toIndex(row1, col1))] = 'n';
        } else {
            if (t == 'n' || r == 'n') id.union(0, size - 1);
            isOpen[id.find(toIndex(row1, col1))] = 'f';
        }
    }
    ...
    private void full(int row, int col) {
        ...
        // last line
            ...
            if (isOpen(row - 1, col)) full(row, col, row - 1, col);
            if (col == 1) {
                if (isOpen(row, col + 1)) full(row, col, row, col + 1);
            } else if (col == n) {
                if (isOpen(row, col - 1)) full(row, col, row, col - 1);
            } else {
                if (isOpen(row, col + 1)) full(row, col, row, col + 1);
        ...
        // left line
            ...
            if (isOpen(row - 1, col)) full(row, col, row - 1, col);
            if (isOpen(row + 1, col)) full(row, col, row + 1, col);
            if (col == 1 && isOpen(row, col + 1)) full(row, col, row, col + 1);
            else if (col == n && isOpen(row, col - 1)) full(row, col, row, col - 1);
        }
        // middle-middle
        else {
            if (isOpen(row - 1, col)) full(row, col, row - 1, col);
