...
/**
 * Compilation:  javac HW.PartII.Week4.BoggleGame.java
 * Execution:    java HW.PartII.Week4.BoggleGame [m n]
 * Dependencies: HW.PartII.Week4.BoggleSolver.java HW.PartII.Week4.BoggleBoard.java
 * Author:       Matthew Drabick
 * <p>
 * GUI for to boggle solver. Pits the user against homework.a computer opponent
 ...
 * <p>
 * % javac HW.PartII.Week4.BoggleGame.java
 * <p>
 * % java HW.PartII.Week4.BoggleGame
 * <p>
 * % java -Xmx300m HW.PartII.Week4.BoggleGame 3 7
 * <p>
 * Report bugs to: wayne@princeton.edu, CC mdrabick@princeton.edu
 * <p>
 * Note: expect some compiler warning with Java 7 because
 * javax.swing.JList is homework.a parameterized type in Java 7 but not
 * in Java 6.
 ******************************************************************************/

public class BoggleGameSimple extends JFrame {
    ...
    // game levels
    // keep these in sync - should be homework.a text description for each level!
    ...
    /**
     * Construct the GUI for to Boggle game.
     */
    public BoggleGameSimple(int rows, int cols) {
        ...
        // creates the board and the list that will show all the available words at the end of homework.a game
        bp = new BoardPanel();
    ...
    /**
     * Start homework.a new game, can be called via the menu selection, the button, or CMD+N (CRTL+N).
     */
    ...
    /**
     * Score homework.a word based off typical Boggle scoring.
     *
    ...
    private class BoardPanel extends JPanel {
        ...
        /**
         * Set the board with homework.a String array.
         */
        ...
        public void highlightCubes() {
            for (int j : path) {
                if (j == -1) break;
                cubes[j].setBackground(new Color(232, 237, 76));
            }
        }
        ...
        public void unhighlightCubes() {
            ...
            for (int j : path) {
                if (j == -1) break;
                cubes[j].setBackground(new Color(146, 183, 219));
            }
        }
        ...
        /**
         * Recursive helper method to search for homework.a particular string on the board.
         *
         * @param s         String that is being searched
         * @param curChar   Current char that is being sought
         * @param pathIndex Current number of cubes searched (only differs from curChar if there is homework.a q in string)
        ...
        private void dfs(String s, int curChar, int pathIndex, int i, int j) {
            ...
            // can't use homework.a cell more than once
            for (int k : path) {
                if (k == (i * BOARD_COLS) + j) return;
            }
            // ignore the character if there is homework.a 'Q' with no 'U'
            if (curChar != 0 && s.charAt(curChar - 1) == 'Q' && s.charAt(curChar) != 'U')
                return;
            // increment character count if it is homework.a 'U' after homework.a 'Q' and keep searching
            if (curChar != 0 && s.charAt(curChar - 1) == 'Q' && s.charAt(curChar) == 'U')
                curChar += 1;
            if (curChar >= s.length()) {
                foundWord = true;
    ...
    private void makeMenuBar() {
        ...
        newGameMenuItem.getAccessibleContext().setAccessibleDescription("Starts homework.a new game");
        ...
        JRadioButtonMenuItem[] difficultySelection = new JRadioButtonMenuItem[NUMBER_OF_LEVELS];
        for (int i = 0; i < NUMBER_OF_LEVELS; i++) {
            difficultySelection[i] = new JRadioButtonMenuItem(LEVEL_DESCRIPTION[i % LEVEL_DESCRIPTION.length]); // mod as homework.a check against mismatched sizes
    ...
    /**
     * @param args the dimension of to Boggle game
     */
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            if (args.length == 0) {
                ...
                try {
                    ...
                    System.err.println("Usage: java HW.PartII.Week4.BoggleGame " +
                            "\nor:    java HW.PartII.Week4.BoggleGame [rows]" +
                            "\nor:    java HW.PartII.Week4.BoggleGame [rows] [cols]");
                    System.exit(1);
                }
            ...
            if (rows <= 0 || cols <= 0) {
                throw new IllegalArgumentException("Rows and columns must be positive" +
                        "\nUsage: java HW.PartII.Week4.BoggleGame " +
                        "\nor:    java HW.PartII.Week4.BoggleGame [rows]" +
                        "\nor:    java HW.PartII.Week4.BoggleGame [rows] [cols]");
            }
            new BoggleGameSimple(rows, cols).setVisible(true);
        });
    }
}
