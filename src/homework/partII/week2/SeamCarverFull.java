...
public class SeamCarverFull {
    ...
    public double energy(int x, int y) {
        ...
        // Two methods to get color: get(), getRGB().
...
//
//        double rx = Math.pow(rgbRight.getRed() - rgbLeft.getRed(), 2);
//        double gx = Math.pow(rgbRight.getGreen() - rgbLeft.getGreen(), 2);
//        double bx = Math.pow(rgbRight.getBlue() - rgbLeft.getBlue(), 2);
//
//        double ry = Math.pow(rgbDown.getRed() - rgbUp.getRed(), 2);
//        double gy = Math.pow(rgbDown.getGreen() - rgbUp.getGreen(), 2);
//        double by = Math.pow(rgbDown.getBlue() - rgbUp.getBlue(), 2);
    ...
    private double energyForDouble(int x, int y) {
        if (x < 0 || x >= width || y < 0 || y >= height) throw new IllegalArgumentException();
...
//        double by = Math.pow(rgbDown.getBlue() - rgbUp.getBlue(), 2);
    ...
    private double switchH(int num) {
        int x = num / height;
        int y = num % height;
        if (y == 0) {
            y = height - 1;
    ...
    private double switchV(int num) {
        int x = num % width;
        int y = num / width;
        if (x == 0) {
            x = width - 1;
...
//        double homework.a = distTo[distTo.length - 1];
